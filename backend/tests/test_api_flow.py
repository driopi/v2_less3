...
def test_mock_mode_autogenerated_answers_flow(client):
    ...
    assert start.status_code == 200
    payload = start.json()
    assert payload["questions"][0]["text"] == "Какую главную цель вы хотите достичь по итогам интервью?"
    assert payload["questions"][1]["text"] == "Какие ограничения по срокам, бюджету и ресурсам критичны на старте?"
    assert payload["questions"][2]["text"] == "Какие ключевые риски вы видите сейчас и как планируете их снижать?"
    preview = client.post(f"/api/session/{payload['session_id']}/mock-answers")
    assert preview.status_code == 200
    preview_payload = preview.json()
    transcripts = [item["transcript"] for item in preview_payload["answers"]]
    assert len(set(transcripts)) == 3
    assert any("preset_round1" in line for line in preview_payload["logs"])
